...
的升级编排层。它面向 2～6 页图片型 PowerPoint，在完整保留 V1 单页质量标准的同时，
通过经过测量的并行执行完成可编辑化重建。
...
- 在向正式输出目录写入文件之前，对 2～6 张幻灯片图片或图片型 PPT/PPTX 页面进行预检。
- 根据 CPU、可用内存、正在运行的 PowerPoint 进程和可用 Codex Worker 槽位，保守估算并发数量。
- 在一个 Codex 任务树中运行最多 3 个页面 Worker；经用户明确授权后，可通过两个任务树运行 4～6 个 Worker。
...
如需运行 4～6 个 Worker，还必须明确授权创建一个辅助 Codex 任务。
...
- 本 Skill 用于编排 2～6 页；单页任务请直接使用基础 V1 Skill。
- 同时运行 4～6 个页面 Worker 需要两个可用的 Codex 任务树，并且必须由用户明确授权创建辅助任务。
